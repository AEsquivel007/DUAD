...
def print_numbers_times_2(numbers_list): # -> The algorithm is:  O(n)
	for number in numbers_list: # O(n)
		print(number * 2) # O(1)

# check_if_lists_have_an_equal
def check_if_lists_have_an_equal(list_a, list_b): # -> The algorithm is:  O(n * m)
...
def print_10_or_less_elements(list_to_print): # -> The algorithm is:  O(n)
	list_len = len(list_to_print) # O(1)
	for index in range(min(list_len, 10)): # O(n)
		print(list_to_print[index]) # O(1)

# generate_list_trios
def generate_list_trios(list_a, list_b, list_c): # -> The algorithm is:  O(a * a * b)
	result_list = [] # O(1)
